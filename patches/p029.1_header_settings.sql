/* ===========================
   p029.1 — header_settings（改修版, DATETIME(6)）
   変更点:
     - レガシー列 checkups_where_sql / checkup_items_where_sql を削除
     - WHERE は docsnap_options_json.where_by_table で指定（任意テーブル対応）
   =========================== */
...
-- =========================================================
-- header_settings — ヘッダ行ごとのスナップ方針
-- （期間= start/end、ターゲット/WHEREは docsnap_options_json を含む）
-- =========================================================
CREATE TABLE header_settings (
  ...
  -- ▼ 並び順と件数（target_table 抽出時）
  order_by_sql          TEXT           NULL                    COMMENT 'ORDER BY 断片（例: visit_date DESC, id DESC）',
  limit_rows            INT            NULL                    COMMENT '抽出上限（NULLで無制限）',

  -- ▼ docsnap拡張（WHEREはここで指定）
  docsnap_options_json  LONGTEXT       NULL                    COMMENT '例: {"where_by_table":{"checkups":"visit_type=\"home\"","checkup_items":"pay_type=\"insurance\""}}',
...
ROW_FORMAT = DYNAMIC
COMMENT='ヘッダ行単位のdocsnap方針。WHEREは docsnap_options_json.where_by_table で指定。';
